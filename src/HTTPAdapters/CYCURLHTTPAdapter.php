...
class CYCURLHTTPAdapter extends CYHTTPAdapter {
  private $ch;
  ...
  public function __construct($host, $port) {
    if(!extension_loaded('curl')) {
      throw new CYException('CYCouchDB cannot use cURL on this system: the PHP cURL extension is not installed.');
    }
  ...
  public function procPacket($method, $url, $data = null, $reqHeaders = array(), $specialHost = null, $specialPort = null) {
    ...
    if($chResponse !== false) {
      ...
      throw new CYException('cURL error #' . curl_errno($this->ch) . ': ' . curl_error($this->ch));
    }
    else {
      throw new CYException('cURL returned false without providing an error.');
    }
   ...
   * @return CYCURLHTTPAdapter Returns $this if talking to the same server
  ...
  private function makeFollowAdapter($parts) {
    ...
    $adapter = new CYCURLHTTPAdapter($parts['host'], $parts['port']);
