...
namespace CYCouchDB;
use CYException;
use CYCouchDBException;
use CYCouchDB\HTTPAdapters\CYNativeHTTPAdapter;
use CYCouchDB\HTTPAdapters\CYCURLHTTPAdapter;
...
class CYCouchDB {
  ...
  public function setHTTPAdapter($type = null) {
    ...
    switch($type) {
      case self::$HTTP_NATIVE_SOCKETS:
        $this->httpAdapter = new CYNativeHTTPAdapter($this->host, $this->port);
        break;

      case self::$HTTP_CURL:
        $this->httpAdapter = new CYCURLHTTPAdapter($this->host, $this->port);
        break;

      default:
        throw CYException("Invalid CYCouchDB HTTP adapter specified: $type");
    }
  ...
  public function login($user, $pass, $type = null) {
    if($type == null) {
      $type = CYCouchDB::$AUTH_BASIC;
    }

    $this->authType = $type;

    switch($type) {
      case CYCouchDB::$AUTH_BASIC:
    ...
    throw new CYException("Unknown auth type for login().");
  }
  ...
  public function decode($decode) {
    if(!is_bool($decode)) {
      throw new CYException('decode() expected a boolean');
    }
  ...
  public function get($url) {
    if(!$this->db) {
      throw new CYException('No database specified');
    }
  ...
  public function delete($id, $rev)
  {
    ...
    if(!is_string($id) || !is_string($rev) || empty($id) || empty($rev)) {
      throw new CYException('delete() expects two strings.');
    }
  ...
  public function put($id, $data)
  {
    ...
    if(!is_string($id)) {
      throw new CYException('put() expected a string for the doc id.');
    }

    if(!isset($data) || (!is_object($data) && !is_string($data) && !is_array($data))) {
      throw new CYException('put() needs an object for data - are you trying to use delete()?');
    }
  ...
  public function post($data, $path = null) {
    ...
    if(!isset($data) || (!is_string($data) && !is_object($data) && !is_array($data))) {
      throw new CYException('post() needs an object for data.');
    }
    ...
    if(is_string($path) && !empty($path)) {
      ...
      throw new CYException('post() needs a string for a path.');
    }
  ...
  public function bulk($docs, $allOrNothing = false) {
    ...
    if(!is_array($docs)) {
      throw new CYException('bulk() expects an array for its first argument');
    }
  ...
  public function copy($srcID, $dstID, $dstRev = null) {
    ...
    if(empty($srcID) || !is_string($srcID)) {
      throw new CYException('copy() got an invalid source ID');
    }

    if(empty($dstID) || !is_string($dstID)) {
      throw new CYException('copy() got an invalid destination ID');
    }

    if($dstRev != null && (empty($dstRev) || !is_string($dstRev))) {
      throw new CYException('copy() got an invalid source revision');
    }
  ...
  public function setDatabase($db, $createIfNotFound = false) {
    if($this->db != $db || $createIfNotFound) {
      if(!is_string($db)) {
        throw new CYException('setDatabase() expected a string.');
      }
  ...
  public function getAllDocs($incDocs = false, $limit = null, $startKey = null, $endKey = null, $keys = null, $descending = false, $skip = 0) {
    ...
    if($incDocs !== false) {
      if(!is_bool($incDocs)) {
        throw new CYException('getAllDocs() expected a boolean for include_docs.');
      }
    ...
    if(isset($startKey)) {
      if(!is_string($startKey)) {
        throw new CYException('getAllDocs() expected a string for startkey.');
      }
    ...
    if(isset($endKey)) {
      if(!is_string($endKey)) {
        throw new CYException('getAllDocs() expected a string for endkey.');
      }
    ...
    if(isset($limit)) {
      if(!is_int($limit) || $limit < 0) {
        throw new CYException('getAllDocs() expected a positive integeter for limit.');
      }
    ...
    if($descending !== false) {
      if(!is_bool($descending)) {
        throw new CYException('getAllDocs() expected a boolean for descending.');
      }
    ...
    if(isset($skip)) {
      if(!is_int($skip) || $skip < 0) {
        throw new CYException('getAllDocs() expected a non-negative integer for skip');
      }
  ...
  public function generateIDs($num = 10) {
    if(!is_int($num) || $num < 0) {
      throw new CYException('generateIDs() expected an integer >= 0.');
    }
  ...
  public function createDatabase($name) {
    if(empty($name) || !is_string($name)) {
      throw new CYException('createDatabase() expected a valid database name');
    }
  ...
  public function deleteDatabase($name) {
    if(empty($name) || !is_string($name)) {
      throw new CYException('deleteDatabase() expected a valid database name');
    }
  ...
  public function replicate($src, $target, $continuous = false, $createTarget = null, $filter = null, $filterQueryParams = null) {
    if(empty($src) || !is_string($src)) {
      throw new CYException('replicate() is missing a source to replicate from.');
    }

    if(empty($target) || !is_string($target)) {
      throw new CYException('replicate() is missing a target to replicate to.');
    }

    if(!is_bool($continuous)) {
      throw new CYException('replicate() expected a boolean for its third argument.');
    }

    if(isset($createTarget) && !is_bool($createTarget)) {
      throw new CYException('createTarget needs to be a boolean.');
    }

    if(isset($filter)) {
      if(!is_string($filter)) {
        throw new CYException('filter must be the name of a design doc\'s filter function: ddoc/filter');
      }

      if(isset($filterQueryParams) && !is_object($filterQueryParams) && !is_array($filterQueryParams)) {
        throw new CYException('filterQueryParams needs to be an object or an array');
      }
    }
  ...
  public function setAttachment($name, $data, $contentType, $docID, $rev = null) {
    if(empty($docID)) {
      throw new CYException('You need to provide a document ID.');
    }

    if(empty($name)) {
      throw new CYException('You need to provide the attachment\'s name.');
    }

    if(empty($data)) {
      throw new CYException('You need to provide the attachment\'s data.');
    }

    if(!is_string($data)) {
      throw new CYException('You need to provide the attachment\'s data as a string.');
    }

    if(empty($contentType)) {
      throw new CYException('You need to provide the data\'s Content-Type.');
    }
  ...
  public function setCache(&$cacheImpl) {
    if(!($cacheImpl instanceof CYCache)) {
      throw new CYException('That is not a valid cache.');
    }
  ...
  public function setStaleDefault($stale) {
    if(!is_bool($stale)) {
      throw new CYException('setStaleDefault() expected a boolean argument.');
    }
  ...
  public function setCookie($key, $value) {
    if(!$key || !is_string($key)) {
      throw new CYException('Unexpected cookie key.');
    }

    if($value && !is_string($value)) {
        throw new CYException('Unexpected cookie value.');
    }
  ...
  public function useSSL($use) {
    if(!is_bool($use)) {
      throw new CYException('Excepted a boolean, but got something else.');
    }
  ...
  public function setSSLCert($path) {
    if($path !== null) {
      if(!is_string($path) || !$path) {
        throw new CYException('Invalid file path provided.');
      }

      if(!is_file($path)) {
        throw new CYException('That path does not point to a file.');
      }

      if(!is_readable($path)) {
        throw new CYException('PHP does not have read privileges with that file.');
      }
    }
  ...
  public function setPathPrefix($path) {
    if(!is_string($path)) {
      throw new CYException('Invalid URL path prefix - must be a string.');
    }
  ...
  private function procPacket($method, $url, $data = null, $headers = array()) {
    ...
    if($data && !is_string($data)) {
      throw new CYException('Unexpected data format. Please report this bug.');
    }
    ...
    if(strtolower($headers['Expect']) === '100-continue') {
      throw new CYException('CY does not support HTTP/1.1\'s Continue.');
    }


    $headers["Host"] = "{$this->host}:{$this->port}";
    $headers["User-Agent"] = "CY/%VERSION%";


    $headers['Accept'] = 'application/json';


    if($this->authType == CYCouchDB::$AUTH_BASIC && (isset($this->user) || isset($this->pass))) {
      $headers["Authorization"] = 'Basic '.base64_encode("{$this->user}:{$this->pass}");
    }
    elseif($this->authType == CYCouchDB::$AUTH_COOKIE && isset($this->authSession)) {
